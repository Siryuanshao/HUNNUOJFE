    ...
    <el-card>
      <div slot="header">
        <div style="display: inline-block; line-height: 45px;">
          <el-page-header @back="goContest" content="Contest Judge Status" style="line-height: 45px;"></el-page-header>
        </div>
      ...
      <el-table
        ...
        <el-table-column
          ...
          label="Problem">
          <template slot-scope="scope">
            <a @click='handleProblemView(scope.row.problemId)'>{{scope.row.problemId | mappingToCharacter}}</a>
          </template>
        </el-table-column>
        <el-table-column
          align="center"
  ...
  export default {
    ...
    methods: {
      init: function () {
        ...
        this.problemId = this.mappingFromDigit(this.$route.query.problemId)
        let contestProblemId = this.$route.query.problemId
        ...
        api.getContestSubmissionList(this.userId, contestProblemId, this.contestId, this.status, this.language, this.currentPage).then(res => {
      ...
      mappingFromDigit: function(value) {
        if(value && !isNaN(parseInt(value))) return String.fromCharCode(64 + parseInt(value))
        else return value
      },
      mappingToDigit: function(value) {
        if(value && isNaN(parseInt(value))) return value.toUpperCase().charCodeAt(0) - 64
        else return value
      },
      handleRoute: function () {
        this.$router.push({
          ...
          query: {
            userId: this.userId,
            problemId: this.mappingToDigit(this.problemId),
            contestId: this.contestId,
            status: this.status,
